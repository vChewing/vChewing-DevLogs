...
> - **v26（2026-09-26，**Phase 261 完工回檢＝判準遷入 Tekkon 暨兩軸命名**）**：事主於 P260 收束後問兩事——「`Typewriter_ZhuyinFuriousAutoChop.swift` 對 Tekkon 的所有擴充是否值得挪到 Tekkon package 內」、以及「`shouldAutoChopZhuyin(byTyping:)` 這個 API 現在對拼音狂打與注音狂打而言的意義是不是統一了？如果是的話，或可重命名為 `shouldAutoChopPhonabets(byTyping:)`」。**評估之結論**：① 該檔**兩個**擴充只有一個是候選——`isZhuyinFuriousTypingModeEffective` **結構上不可能搬**（它讀 LibVanguard 之 `isFuriousTypingModeEffective`）；② 判準可搬，其依賴面**全在 Tekkon 之既有 `public` API**（逐一實查 `isEmpty`／`receiveKey`／`getComposition`／`parser`／四槽／`Composer.init(arrange:)`／`SyllableIndex`）；③ 測試靶**不沾 Tekkon 以外之組件**（`@testable import LibVanguard` 實為空載，逐符號掃命中 0）；④ §3.3 之紅線**按其原文不禁止此事**（見該節之範圍釐清）；⑤ 意義**在「決策」層統一、在「乘載」層刻意不統一**——拼音側回傳結構（`committedReadings` ＋ `remainingRomaji`）、注音側回布林，因為注音之讀音**本就在注拼槽內**（呼叫點以 `phonabetKeyForQuery` 自取），而拼音之切分必須被建構。**★ 兩軸命名規則（事主裁定，本 phase 落成）**：**名字指「模式／鍵盤家族」者留 `Zhuyin`；名字指「切分機制與其提交物」者用 `Phonabet`**（拼音側則用 `Pinyin`，因其提交物是 romaji）。據此：`shouldAutoChopZhuyin` → `shouldAutoChopPhonabets`、`zhuyinAutoChopSlots`／`zhuyinAutoChopHighestFilledSlot` → `phonabet…`（回傳的正是四個 phonabet 槽，故更準）、`performZhuyinAutoChopIfNeeded` → `performPhonabetAutoChopIfNeeded`（它提交 `phonabetKeyForQuery`）；**不動者**：其孿生 `performPinyinAutoChopIfNeeded`（運送 romaji、以 `replacePinyinBuffer` 重建，**是另一種機制**）、`InputHandlerTests_ZhuyinFurious.swift`（驗的是**模式**）、`isZhuyinFuriousTypingModeEffective`（模式真值）。**實查之裁定**：兩者提交之物同為**無調注音詞幹**（`PinyinTrie` 之 value 即 zhuyin 讀音 ⇒ `committedReadings` 之文件即自陳「前段注音讀音鍵」），故機制確為族中立。**交付**：`Sources/Tekkon/Tekkon_PhonabetAutoChop.swift`（71 行，新檔，兩個助手首次成為**真 `private`**）；`Tests/TekkonTests/TekkonTests_PhonabetAutoChopPredicate.swift`（遷入並改名，去空載之 `@testable`、`#filePath` 少一層）；刪 `Typewriter_ZhuyinFuriousAutoChop.swift`，其閘門歸位至 `InputHandler_FuriousResegmentation.swift` 之內、緊接通用閘與拼音閘（**該檔為「消費 `typingMode` 之模式真值」，非打字機之物**）；五處交叉引用同步（兩支 LibVanguard 靶之檔頭、判準靶之 `struct`／`@Suite`／兩支測試函式名）。**★ Tekkon 檔案頭新增一則必要之註**：權威規格（逐條理由、四則對照實例、三條已知界線）住本規劃書 §3.2，而**該檔不在 Tekkon 套件內** ⇒ 實作以摘要自持，且**雙向綁定**：不得只動實作而不動正本、亦不得反之。**驗證**：**634 支／0 失敗、exit 0**（動工前後同值，僅歸屬位移：`LibVanguardTests` 260 → **256**、`TekkonTests` 53 → **57**）；兩倉逐檔 SHA-256（`Sources` ＋ `Tests`，排除 `.DS_Store`）**211 檔全同、0 不符**、六份 manifest ＋ `makefile` 零差異、`git mv` 保留兩側之改名歷史；**鏡像側獨立複跑**同值。**Swift 5.10 之裁定（事主明示）**：**所有 Swift 單元測試釘死於 6.4＋ 主分支，5.10 不負責任何單元測試**（其 `Package@swift-5.10.swift` 實查未宣告任何 `testTarget`；而 Swift Testing macro 需外源線上依賴、會帶來額外維護壓力）⇒ 靶之遷移**無覆蓋損失**、manifest 無須改動。**卷之歸屬**：本 phase 之記錄住新卷 `Reqs4LLM/Archive_P201-P300/Reqs_0261-0270.md`（前卷 `Reqs_0251-0260.md` 於 P260 之後即滿 10 個 Phase，依〈Reqs4LLM 分卷歸檔註記〉開新卷）。**受修訂之小節**：文首〈修訂沿革〉（本行）、**§3.3（新增〈範圍釐清（v26）〉）**、§8.6（live 指標之新址與新名）。**未重編者（刻意）**：本卷各 phase 之歷史記錄；惟凡「以舊名描述**現況**」者已改、並以「P255 原名…」之形式保留該名之出處。**無後續 phase 待排。** **（同日之 CI 追補，`ad55c7c`／`4aa278bc`）**：交付後 `linux`／`WinNT` 兩條 workflow 於 `ad55c7c` 轉紅（同 commit 之兩條 macOS workflow 皆過）。**診斷**：Linux 之判準靶①「不得在單一音節內誤切」誤切 **7366** 次、全數「鍵＝反引號」；Windows 之 `AutoChopCorpus.rows` **整批為空**（②受檢交界 0）。**★ 根因：靶自建了按鍵域，未自資料取用排列之按鍵域。**① `candidateKeys`（反推鍵表之**探針集**）誤收反引號與空格——兩者**不是任何出廠排列之按鍵**，素材檔內只作「本排列無此鍵」之標記（`` `NULL``、源自 `__` 之尾端空格）⇒ 反推把它們登記成某注音符號之按鍵，遂由合法讀音之前綴生成出**不可鍵入**之鍵序；**判準對「非按鍵」之反應本即無定義**，故各平台各自為政。② 語料閘原為 `!cell.hasPrefix("`")`（過寬）⇒ 改為**逐格校驗鍵面字元**。③ Windows 之載入失敗原為**靜默**（只以魔數下界 `steps > 29_000`／`checked > 100_000` 間接失手）⇒ 改報「未濾列數 vs 過濾後列數」、下界改為**結構量**（語料須成規模、解析損失 ≤ 5%、交界數 > 0），且該檔之讀取路徑改走 `(#filePath as NSString).deletingLastPathComponent`（不再用 Foundation 之上層目錄運算＋手接分隔符）。**迴歸釘**：斷言「反推所得之按鍵一律須為鍵面字元」——放回反引號即轉紅。**★ 通則（登錄）**：**(a) 回歸靶之輸入域必須自資料導出，不得由靶自建**（靶自建之鍵域含「非按鍵」即等於把無定義之行為當規格來測——此為「靶綠而 CI 紅」之典型：缺陷在**測什麼**）；**(b) 語料類靶須能分辨「沒讀到」與「讀到了但濾掉幾列」**；**(c) 大型靜態初始值之複雜推導會撞編譯器**（`compactMap` 閉包配切片令 swift-frontend 6.4 崩潰於 `TypeCheckPrimaryFileRequest`，改樸素迴圈即過）。**判準本身一字未動**（本次只動靶）。**受修訂之小節**：文首〈修訂沿革〉（本行）。**未以 amend 併入原提交**（原提交已推送且 CI 已於其上跑過）。 **（追補之收束）**：Linux 於第 2 筆提交後轉綠；**Windows 於第 3 筆提交（`ad55c7c`／`4aa278bc`）後轉綠、四條 workflow 全綠**。Windows 之真因在「改路徑運算」之後**仍為整批讀不到**，而**新加之行數斷言報出 `rawRowCount == 0`** ⇒ 檔案根本沒開成功（連錨點搜尋皆未執行）。該次一併修入三項而收斂：① 候選路徑清單（四條接法依序試、全數回報）；② 行尾正規化（`\r\n` → `\n`）；③ 候選迴圈不再於「讀到但解不成 UTF-8」時中斷。**已知界線**：三項同批入版 ⇒ **無法逐項歸因**。**通則（補）**：**(d) 非 Darwin 之載入失敗先讓它會說話再修它**（否則症狀只是某個魔數下界失手）；**(e) 「症狀已定位」≠「根因已定位」**——修法未經複驗者，措辭不得由「疑似」升為「已修」（本案第 2 筆提交之訊息即犯此，已在第 3 筆更正）；**(f) 同批修多項須明寫無法逐項歸因**。
...
> **範圍釐清（v26，P261 實查）**：本紅線之**實質**是後半句——「不得在 `receiveKey` 內做任何自動提交」；前半句之「知曉『狂打』概念」依其自陳之理由（Tekkon 是**無狀態偏好、無會話概念**之音節組裝器）指的是**行為**，而非**命名**。兩項反例實查如下：① `PinyinAutoChopResult`（`Tekkon_SyllableComposer.swift:276`，文件自陳「**試算**是否應自動 chop」「若本拍不應觸發自動 chop 則回傳 nil」）——**在 Tekkon 內回傳自動 chop 之判準與建議切分**；② `allowsExtendedRomajiBuffer`（同檔 `:89`）之註解直書「狂拼模式」。⇒ **遷入 P261 之 `shouldAutoChopPhonabets` 不破此紅線**：它回傳裁決、不改任何狀態、不在 `receiveKey` 內。**紅線仍然有效者**：凡「Tekkon 自己動了使用者的組字狀態」之提案（例如在 `receiveKey` 內自動提交、或讓 Tekkon 讀取偏好／會話）一律不許。
...
**新增**（自 **P261** 起判準住在 `Sources/Tekkon/Tekkon_PhonabetAutoChop.swift` 並名為 `shouldAutoChopPhonabets`；**執行端**留在 `Typewriter_BPMFFullMatch.swift`、與 `performPinyinAutoChopIfNeeded` 並列，【P261 新址】：

```swift
/// 注音狂打：本鍵是否應先把當前音節固化進組字器。
/// - Returns: 已固化則 `true`；不應固化則 `nil`（呼叫方照常把按鍵送入注拼槽）。
private func performPhonabetAutoChopIfNeeded( // P255 原名 performZhuyinAutoChopIfNeeded
  inputText: String,
...
| **落點** | `Typewriter_BPMFFullMatch.swift`（新 `performPhonabetAutoChopIfNeeded`（P255 原名 `performZhuyinAutoChopIfNeeded`），與 `performPinyinAutoChopIfNeeded` 並列；呼叫點在 `receiveKey` 之正前方）；`Tests/LibVanguardTests/InputHandlerTests_Cases1.swift`（或新增檔）加入新測試 |
