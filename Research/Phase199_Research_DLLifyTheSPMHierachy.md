...
- **E4（`.dynamic` 與 SwiftPM 測試靶）**：把 `OSNeutralAssembly` 暫時 `.dynamic`（或新開一個 scratch 套件），確認 `swift test`、`--no-parallel`、以及 Ubuntu／Windows CI 的對應行為（`.so`／`.dll` 搜尋路徑）。
...
| 資源查找 | `Packages/vChewing_MainAssembly4Darwin/Sources/MainAssembly4Darwin/LXManager/BundleAccessor.swift:35-44`（`Bundle.currentSPM` 候選集）；SwiftPM 自動生成 accessor 位於 `.build/out/Intermediates.noindex/<Target>.build/<Config>/<Target>-t.build/DerivedSources/resource_bundle_accessor.swift` |
