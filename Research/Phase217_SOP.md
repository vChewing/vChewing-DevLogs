...
| 項 | 值 | 為什麼（一行實測） |
|---|---|---|
| **工具鏈** | `LEGACY_TOOLCHAIN = $(firstword $(wildcard /Library/…/swift-5.10.1-RELEASE.xctoolchain $(HOME)/Library/…/swift-5.10.1-RELEASE.xctoolchain))`（自動擇一） | 只有 5.10 會挑到 `Package@swift-5.10.swift`（§三）。**2026-09-16 更正**：原稿寫死 `$(HOME)` 那份，但官方安裝器把 5.10.1 放 `/Library`、`swiftly` 放 `$HOME`；**兩者是同一發行版、識別子相同**，一旦並存，Xcode 會拒絕註冊任一份，所有 `xcodebuild` 於套件解析前即敗（rc=65），而「刪 `$HOME` 那份」又會連帶打死原寫死該路徑的 legacy 目標。故全倉 **23 份** makefile 一體改為自動擇一：優先 `/Library`（不受 `swiftly uninstall` 影響），缺了才落回 `$HOME`。取證、實測與處置見 **P218〈十五〉** |
| **SDK** | `LEGACY_SDK = /Library/Developer/CommandLineTools/SDKs/MacOSX13.3.sdk`（2026-09-16 改綁；原值為 Xcode 15 內那份同名 symlink） | **SDK 錯＝312 個掩蓋式錯誤**：`module '_c_standard_library_obsolete' requires feature 'found_incompatible_headers__check_search_paths'`、`unknown argument: '-target-arch-variant'`、`could not build module 'Darwin'`——沒有一條指向真正的病灶；正解是 Xcode 15 的 MacOSX13.3.sdk。（**2026-09-16 追記**：該 SDK 之真身其實在 CLT，`LEGACY_SDK` 已改綁 `/Library/Developer/CommandLineTools/SDKs/MacOSX13.3.sdk`——落點 21 份 makefile；且該 SDK 屬 legacy，現行 CLT 27.0 已把 macOS 11／12／13／14 之 SDK 一律改成「移除包」，換機時須自 macOS 13.3 之 CLT／Xcode 14.3 自行備妥。事主提問與完整查明見 P218 之附註） |
...
| **部署目標** | `LEGACY_TRIPLE = x86_64-apple-macosx10.9` | **2026-09-16 定案：地板收斂至 10.9**（P218〈十六〉；鐵則——legacy 分支存在的唯一意義就是支持 macOS 10.9）。此前一稿記為「**10.9 不存在**：`Data` 標為 macOS 10.10 起可用、`-target …10.9` 建不出東西（連 `@backDeployed` 同名 shim 都平替不了）」，該「`Data` 說」**已經重測推翻**（P218 以正式配對冷啟建置全閉包：`Build complete!`、0 筆 `error:`）；10.9 之真正代價是 5 支 AppKit API（`addChild`／`placeholderString`／`sendsWholeSearchString`／`sendsSearchStringImmediately`／`secondaryLabelColor`）共 **57 筆** availability 錯誤、**全落在 `vChewing_SettingsUI` 兩檔**，已依 `vChewing-OSX-Legacy` 之 10.9-compliant 體例結清。根 `Makefile` 之 `LEGACY_X86_TRIPLE` 與 20 份逐包 makefile 之 `LEGACY_TRIPLE` 一併翻轉。 |
| **arm64 slice** | `LEGACY_ARM_TRIPLE = arm64-apple-macosx11.0` | arm64 沒有 11.0 以下的 macOS，故唯一一支不以 10.9 為目標者；`releaseLegacy` 之 universal 產物即此二 triple 之 `lipo`。該支線**不摻** libArcLite（archive 僅 x86_64 一個 slice）。 |
...
```bash
# 5.10.1 toolchain：兩處「至少有一份」即可（`LEGACY_TOOLCHAIN` 會自動擇一）；
# 兩份並存反而是故障態——同一識別子會令 Xcode 拒絕註冊，所有 xcodebuild 當場即敗。
...
ls -d /Library/Developer/CommandLineTools/SDKs/MacOSX13.3.sdk   # 2026-09-16 改綁（原為 Xcode 15 內那份 symlink）
...
**2026-09-16 追記**：使用者空間那份（`swiftly` 所置）已由事主移除，僅餘 `/Library` 那份；自動擇一後 legacy 路徑照常。詳見 §2.1 之更正與 P218〈十五〉。
...
# 工具鏈自動擇一：官方安裝器放 `/Library`、`swiftly` 放 `$HOME`，同一識別子並存會令 Xcode
# 拒絕註冊而所有 `xcodebuild` 於套件解析前即敗，故優先系統那份、缺了才落回使用者空間那份。
...
1. **`-Xswiftc -target -Xswiftc "$(LEGACY_TRIPLE)"` 絕不可省。** SwiftPM 5.10 會把 `--triple` 的**版本部分丟掉**、一律抬到自身地板（x86_64 為 `-target …-macosx10.13`）；宣告 `platforms: [.macOS(.v10_10)]` 也一樣被抬；`--triple` 單獨用無效；arm64 更被 linker 釘在 `minos 11.0`。**唯一有效的手段是 `-Xswiftc -target`（同一個 `-target` 後出現者勝）**——實測 `.o` 與最終二進位元組的建置版本確實是 `$(LEGACY_TRIPLE)` 之值（現為 10.9）。
...
**`LEGACY_TRIPLE` 為 `x86_64-apple-macosx10.9`**（§2.1；2026-09-16 自 10.10 收斂，見 P218〈十六〉）：它是**編譯期 triple**，與 `platforms: nil` 各司其職——triple 決定「以哪個版本的 SDK API 為編譯基準」，`platforms: nil` 決定「manifest 不主動宣告平台下限」。
...
# 工具鏈自動擇一（2026-09-16）：官方安裝器放 `/Library`、`swiftly` 放 `$HOME`，同一識別子並存
# 會令 Xcode 拒絕註冊而所有 `xcodebuild` 於套件解析前即敗。故優先系統那份、缺了才落回 `$HOME`。
LEGACY_TOOLCHAIN ?= $(firstword $(wildcard \
	/Library/Developer/Toolchains/swift-5.10.1-RELEASE.xctoolchain \
	$(HOME)/Library/Developer/Toolchains/swift-5.10.1-RELEASE.xctoolchain \
	))
...
2. **`-Xswiftc -target -Xswiftc "$(LEGACY_TRIPLE)"` 不可省**——SwiftPM 5.10 會丟掉 `--triple` 的版本部分並抬到自身地板（10.13），只有 `-Xswiftc -target` 能真正壓到 10.9（§2.3 要點 1；2026-09-16 收斂，見 P218〈十六〉）。
...
1. **5.10 側一律 static**：5.10 側的宿主是 macOS 10.9 級 legacy app，其 ARC 執行期由 libArcLite 提供，而 **dylib 必須在建置期就專門摻進該靜態庫**（不能像 app 那樣由宿主統一連結），故本側不產出任何動態庫。
...
　**`#Preview` 之機理（為何非圈不可）**：`#Preview` 是 **SwiftUI 的巨集**，其展開結果**需要 macOS 14 級的 preview 執行期**，且展開本身**需要 Xcode 的 `PreviewsMacros` 外掛**——在 5.10 側（OpenSource toolchain、部署目標 10.9、SDK 13.3）**必失敗**。它特別容易漏，因為它常躲在 preview-only 碼裡、看起來「不影響出貨」；**本倉實測有 17 處、分布於 17 個檔**（§5.4 之（一））。
...
| `Data` | 型別本身標為 macOS **10.10** 起可用 | **已作廢**：P218 冷啟建置實測 rc=0，`Data` 之 10.10 標記不構成部署目標之下限；見 P218〈十六〉 |
...
> **✅ 一項既存測試不穩定——已修復（2026-09-15 深夜，`208554e LibVanguard // Patch unit test issues.`，16 檔、+490/−412）**：本節原先列報「聚合體 6.4 側 `swift test --no-parallel` 之 `test_IH400_MixedAlnumKanjiInputTest_Izanami(_:_:)` 間歇失敗（四次連跑之 issue 數 4／3／0／0）」——根因為**測試彼此踩踏行程內全域狀態**（`LXFacade.factoryTrie`／`lxCassette`、`PrefMgr.shared`、`SessionHost.shared`）＋該測試是全靶唯一「跑遍整張注音表卻未把 `asyncLoadingUserData` 壓成 `false`」者。**修法與驗收見 §十二 R23 之 (b)(c) 與 `Phase216_PostResearch.md` §十四**：`LibVanguardTests` 收進單一 `.serialized` 根 suite、該測試補上同步載入、`HomaTests` 之堆積哨兵改為多次取樣；**兩倉各以 `swift test --no-parallel` 連跑 5 次全綠（535 項、0 issue）**。**故本節上述「以兩次連跑皆綠為準」之臨時判準不再需要**；`--no-parallel` 與 `.serialized` 兩道防線仍**缺一不可**（理由見 R23 末段）。**與 P217 之 5.10 手術無關**之判斷不變（中招者位於聚合體、6.4 側）。
...
| 倉 | HEAD | 變動 |
|---|---|---|
| `vChewing-macOS` | `7d920ec3`（`MainAssembly // Swift 5.10 compilability.`；**其後再經兩次 rebase，訊息不變、現值即此**） | 收工時 **clean**；2026-09-16 之追加 43 項（42 份封堵檔 ＋ `Packages/vChewing_MainAssembly4Darwin/Package@swift-5.10.swift`）**已隨該倉 rebase 入庫——現值 clean**（已複驗：該 manifest 之 lexicon 相依與兩支 plugin、以及 42 份封堵檔皆在 `HEAD` 之樹內） |
| `vChewing-LibVanguard` | `208554e`（`LibVanguard // Patch unit test issues.`） | 收工時 **clean**；2026-09-16 之追加 4 項（聚合體與 `Deps/VanguardSwiftExtension` 之封堵檔各 2 份）**已隨 rebase 入庫——現值 clean** |
| `vChewing-DevLogs` | `4429504`（`P217.`）——**該 hash 其後已隨該倉之 rebase 消失、於任何 ref 皆無對位可尋；P216／P217 兩筆登記之內容現摺於 `a046d0c`（`P126-P128.`；該筆即 `83b473c`／`d24a0cb` 經同日再一波 rebase（squash）後之現值——P216～P218 之文書自此與 P126～P128 之內容同在一筆）** | **clean**（`Research/Phase216_PostResearch.md`／`Research/Phase217_SOP.md`／`Reqs4LLM/Archive_P201-P300/Reqs_0211-0220.md` 三者已入庫） |
...
**hash 複測（2026-09-16，事主三倉各自 rebase 之後；同日晚些之再一波 rebase 後復再複測一次，下表為後者）**：本檔與 `Phase216_PostResearch.md`、`Reqs_0211-0220.md` 中所引之「現行」hash 已全數複測更新（`vChewing-LibVanguard`：`208554e`／`1c60407`；`vChewing-macOS`：`7d920ec3`／`bcaebc46`／`418015a3`／`07575402`；`vChewing-DevLogs`：`4429504`／`942557d`——**惟此兩筆其後已隨該倉之 rebase 消失、於任何 ref 皆無對位可尋；其內容現摺於 `a046d0c`（`P126-P128.`；該筆即 `83b473c`／`d24a0cb` 之現值，亦即其後第三、四波之結果——P216～P218 之文書與 P126～P128 之內容現同在一筆）**）。三倉於該次複測時皆 **clean**。依 §十二 R12，hash 仍會隨下一次 amend／rebase 失效——**判準一律以「訊息 ＋ 小節主題」為準**。
...
## 附錄：P216–P218 hash 對照表（2026-09-16 盤點）

**為何有此附錄**：P216 之後，四個倉的歷史被反覆 amend／rebase——尤以 2026-09-16 把 19 份逐包 `makefile` 一檔一 commit 再摺回既有歷史那次為甚——令 P216／P217／P218 文書中所引之 hash **絕大多數已不再是所在倉現行歷史的成員**。依 §十二 R12「判準以訊息 ＋ 小節主題為準」，這本是預期內的磨損、不是敘事錯誤；惟磨損到九成時，「拿 hash 去查證舊記載」這條路等於斷了。本附錄是一次性盤點之**快照**，把每個舊 hash 對回其現行值，使舊記載重新可查。**2026-09-16 補記**：本附錄於同日再遇一波 rebase（19 份逐包 `makefile` 之一檔一 commit 沿革再摺回其本源 commit），B 表右欄之「現行值」遂又有 **13 筆**失效——已就地把這 13 筆移列左欄、右欄改記該次 rebase 後之實測值（總帳同步更新）；〔使用須知 1〕之警語——本附錄自身即為快照、會隨下一次 amend／rebase 失效——對補記後之本文依然成立。**同日晚間再記**：同日又遇一次 rebase（`vChewing-macOS` 之 `5ae0ddc0` 之後三筆 `LLMDocs`／`README` 沿革被摺回、`Repo // Assembles…` 一筆被重作），B 表右欄再失效 **1 筆**（`7f23b1b1` → `b3b19a77`）；該筆 author date 未變，屬甲級對位。B 表、判定基準之配比與總帳已同步更新；被一併重寫的 `4e076d18` → `3d376bf0`（`LLMDocs // …`）與 `8d6f3eab`／`ca6342ca` → `75ab0219`（`README // …`）兩線，因文書中從未引用其 hash，不進 B 表（該批之值其後再失效一次，見下）。**同日又一記（第三波）**：`vChewing-macOS` 之 `5ae0ddc0` 之後再度改寫——除原先三筆外，另**插入新筆 `b48b07b5`**（`Installer // Patch a null handler assignment to the retry timer.`，本 phase 第五輪之崩潰修補），故 B 表右欄再失效 **1 筆**（`b3b19a77` → `5793727b`；該筆今含第五輪之 `Makefile`／lexicon 建置改動與三份指引檔）；`vChewing-DevLogs` 之 `83b473c` 先經一波 amend 成 `d24a0cb`（author date 與訊息皆未變，屬甲級），隨即又經一波 **squash** 成 `a046d0c`（訊息 `P126-P128.`）——P216～P218 之全部文書登記自此與 P126～P128 之內容同在一筆。**三條線之值鏈至此為**：`7f23b1b1` → `b3b19a77` → `5793727b`（`Repo // Assembles the app bundle for legacy targets.`）；`4e076d18` → `3d376bf0` → `95f6ff84`（`LLMDocs // …`）；`8d6f3eab`／`ca6342ca` → `75ab0219` → `c7812115`（`README // …`）。B 表、判定基準之配比、總帳與相異 token 數已再同步。**同日收尾記（第五波：非 rebase）**：該批工作之最終 commit 為 `vChewing-macOS` 之 `ec729322`（`README // State something about Swiftly.`）＋ `2bee7640`（`Makefile // Cancel the needs of putting macOS 13 SDK into Xcode 15.`，含 `LEGACY_SDK` 改綁 CLT 與 build plugin 之平台模板 fallback）——**本波係新增 commit、未改寫歷史，故本附錄之 B 表、右欄現值、判定基準配比與總帳一概未失效**（僅 `5ae0ddc0` 之前者仍如各表所記）。**同日再記（第六波）**：`vChewing-DevLogs` 之 `P126-P128.` 一筆又經一次 amend（訊息與 author date 均未變），其值鏈自此為 `83b473c` → `d24a0cb` → `cd4e4e9` → `a046d0c`；本附錄各表之引用已就地刷新，B 表本身不受影響。

**盤點範圍**（五份文件）：`Research/Phase216_PostResearch.md`、`Research/Phase217_SOP.md`、`Reqs4LLM/Archive_P201-P300/Reqs_0211-0220.md`（P216～P218 段，即 832 行起）、`DevReqsHistory.md`（P216–P218 之六列）、`KnowledgeMemo4LLM.md`（§10.5）。合計 **155 處、54 個相異 token**（補記後，B 表之相異 token 增為 69——見下）。

**判定基準**：該 hash 是否為該倉 `refs/heads/main` 之**祖先**（＝「還指得到東西」）。單看「物件是否存在」不夠——reflog 殘骸與滯後的 remote-tracking ref 都會讓已死的 hash 依然查得到。對位分兩級：**甲級——(author date ＋ commit message) 雙條件相符**（rebase 不會改這兩者），53 筆中佔 45 筆；**乙級——僅以訊息對位**（該筆曾被「重作」而非單純 amend，author date 已變，故只剩訊息可依），佔 8 筆，於 B 表標 `†`。乙級仍無歧義：那些訊息在各自倉的現行歷史裡只餘一個世代。
...
|---|---|---|
| 仍在 `main` 歷史內 | **6** | 無須更動 |
| 已懸空，可對位到現行 commit | **53** | 查 B 表即得現行值 |
| 已懸空且**無任何對位** | **3** | 見 C 表；其中兩筆為 DevLogs 之完工登記，內容已摺進 `a046d0c` |
| 非 commit（雜訊） | **7** | 本非 hash，見 D 表 |

### A. 仍有效者（6）

| hash | 倉 | 訊息 |
|---|---|---|
| `1424601` | `vChewing-LibVanguard` | `LibVanguard // Dylibify VanguardSwiftExtension.` |
| `6fff46c1` | `vChewing-macOS` | `LibVanguard // Dylibify VanguardSwiftExtension.` |
| `89af456f` | `vChewing-macOS` | `SwiftExtension // Swift 5.10 compilability.` |
| `24364eb1` | `vChewing-VanguardLexicon` | `批次新增語彙 - 20260916` |
| `bc460de0` | `vChewing-OSX-legacy` | `UpdateSputnik & Installer // Refactor how to know the distro is legacy.` |
| `f8867ee1` | `vChewing-OSX-legacy` | `Repo // Rearrange the use of CGRect extensions.` |

`1424601` 與 `6fff46c1` 是同一筆工作在兩倉的對位，兩者皆仍有效。

### B. 舊 → 新 對照表（53）

**`†` 記號**：該筆之 author date 已與現行值不同——它被「重作」過而非單純 amend／rebase，故只剩訊息可依（乙級對位）。無記號者為 (author date ＋ message) 雙條件相符（甲級）。兩級皆無歧義：這些訊息在各自倉的現行歷史裡都只餘一個世代。

#### `vChewing-LibVanguard`

| 已懸空（舊記載） | 現行值 | 訊息（定鑑用） |
|---|---|---|
| `a8d5fde` `0681e14` `189a76d5` `a4db83c` | `13097e0` | `SPM // Starting working on Swift 5.10 compilability.` |
| `28d491f` `6e5f19f2`† `b757b2a`† `5b6e9c3`† | `ac781c1` | `SwiftExtension // Swift 5.10 compilability.` |
| `2a4466d` `2bbce67` `2bef551` `2bef551d43b6e9d81d2616b41b2cf1cd0a0ec1e7` `a2160db8` `854f5d6` | `826a744` | `LibVanguard // Swift 5.10 compilability.` |
| `45bb0ac` `8864064` | `208554e` | `LibVanguard // Patch unit test issues.` |
| `6609f5b` `95a609c`† `f4a162b` | `1c60407` | `SPM // Block Swift 6.2 and 6.3 toolchains.` |
| `d4a4718` `c687dd4` | `6275558` | `LibVanguard & SwiftExtension // Rearrange the use of CGRect extensions.` |

#### `vChewing-macOS`

| 已懸空（舊記載） | 現行值 | 訊息（定鑑用） |
|---|---|---|
| `121ef1a3` `d920059e` `ddb217f6` `df09641e` `b6d8e26f` | `3e72cddb` | `LibVanguard // Swift 5.10 compilability.` |
| `eeba341d`† `52200afb`† `d0ff1138`† | `89af456f` | `SwiftExtension // Swift 5.10 compilability.` |
| `14ee2dce` `19483b3b` | `5ae0ddc0` | `Repo // Rearrange the use of CGRect extensions.` |
| `bd35199b` `86e19319` | `ebd17348` | `LibVanguard & SwiftExtension // Rearrange the use of CGRect extensions.` |
| `3ecbe770` `7553a687` `f0f06f73` | `7d920ec3` | `MainAssembly // Swift 5.10 compilability.` |
| `284dd920` `db5fb265`† `940af034` `7f23b1b1` `b3b19a77` | `5793727b` | `Repo // Assembles the app bundle for legacy targets.` |
| `d033e90c` `1cf00f97` | `09c97ebc` | `SettingsUI // Swift 5.10 compilability.` |
| `48ff88ef` `307be98b` | `07575402` | `Repo // Prepare for Swift 5.10 compilability works.` |
| `588d1b8e` `d88a423c` `4070c7e5` | `418015a3` | `Repo // Block Swift 6.2 and 6.3 toolchains.` |
| `4125707e` `35922297` | `461162be` | `LibVanguard // Sync licenses from the dedicated repo.` |
| `cad648e3` `448821b0` | `bcaebc46` | `LibVanguard // Patch unit test issues.` |
| `a5384aeb` | `60eb5f33` | ``LXMgr // Call `savePOMData` on MainActor.`` |

`eeba341d`／`d0ff1138`／`52200afb` 之現行值 `89af456f` **本身仍有效**（見 A 表）——這是唯一「舊值死了、新值還在」的一條線。

**`df09641e` 的特例**：它並未消失，而是滯留在 `refs/remotes/all/main` 這個**未跟上 force-push** 的 remote-tracking ref 上；`refs/heads/main` 上是 `3e72cddb`。`Phase216_PostResearch.md:376` 與本檔開頭（§一 之前的數值快照）把它當「彼時的 `git log` 頭」引用——作歷史陳述不算錯，但它已不是任何意義上的現行值。

### C. 無任何對位者（3）

| hash | 倉 | 文件原本怎麼寫 | 現況 |
|---|---|---|---|
| `942557d` | `vChewing-DevLogs` | `P216.`——「本 phase 的正式完工登記已於同日完成」（`Phase216_PostResearch.md` 檔頭、§13.6.6、文末修訂紀錄） | 該 commit **不存在**。內容已摺進 `a046d0c`（`P126-P128.`，即 `83b473c`／`d24a0cb` 之現值）：該倉之 P215 以後只有 `239591b`（`P215.`）→ `7add503`（`README // Patch license.`）→ `a046d0c`，而 P216／P217／P218 之三次登記之文書即在此最後一筆（與 P126～P128 之內容同筆） |
| `4429504` | `vChewing-DevLogs` | `P217.`（本檔 §十三 收工狀態表、§十三 之 hash 複測） | 同上 |
| `376140a1` | `vChewing-OSX-legacy` | 舊值、已不可達（`Reqs_0211-0220.md:1344`） | 文件說法**正確**：該筆在 legacy 現行歷史無同訊息之對位，reflog 亦已回收 |

### D. 非 commit 之雜訊（7）

- **GitHub Actions run ID**：`34934096396`、`34934096352`、`34934096340`
- **GitHub Actions job ID**：`104268203553`、`104268203072`、`104268203258`
- **工具鏈識別子片段**：`5101202406041a`（出自 `org.swift.5101202406041a`）

皆出自 P216 之 CI 失敗取證（那三條 job 日誌的 URL），**本非 commit hash**，毋須處理。

### 使用須知

1. **本附錄自身即是一次快照，會隨下一次 amend／rebase 失效**——與它要修補的對象同病。查證時仍以「訊息 ＋ 小節主題」為第一判準，A～C 表只作輔助。
2. **未更動任何既有記載**：舊 hash 一律原樣留在原處（它們是「當時記了什麼」的證據），本附錄不代為替換；惟 C 表之兩筆 DevLogs 登記，已在 `Phase216_PostResearch.md` 之檔案與本檔 §十三 就地加註其消失。
3. 產出之法：對五份文件以 `\b[0-9a-f]{7,40}\b` 抽取 token，逐 token 於五倉試 `git rev-parse`，再以 `git merge-base --is-ancestor <hash> refs/heads/main` 判現行性，最後以 (author date, message) 對位。
