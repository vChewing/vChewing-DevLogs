...
- **落地狀態（2026-09-14，Phase 213 更新——同一設計在另一倉的第二次落地）**：`vChewing-LibVanguard` 已整組改為與 `vChewing-macOS/Packages/vChewing_OSNeutral_LibVanguard` 同源的聚合包，出貨單一動態產品 `Vanguard`／`libVanguard.dylib`（`package.name` 取 `LibVanguard`、**product 名取 `Vanguard`**，故此倉不會產生 `libLibVanguard.dylib`；模組名與 target 名一律不變）。此即本研究 §9.3 路線 (d)「另立一個 `.dynamic` product 讓它把整個閉包吸進去」在**非 app 倉**的落地，且該倉原本的 `LibVanguard` 聚合 target ＋ `Sources/_Modules/*` 舊佈局已全部退場。**與 Phase 210 的關鍵差異**：Phase 210 的 dylib 活在 `vChewing-macOS` 的 app 出貨管線內（`Contents/Frameworks`、rpath、簽章、公證），而 LibVanguard 這一側目前**只做到「能編出 dylib 並通過全部單元測試」**——其向前建置能力（消費者改吃此倉）依事主明示屬後續 phase。資源面不受影響：本倉的資源 bundle 名為 `LibVanguard_*.bundle`（SwiftPM 之 bundle 名取套件名），兩處硬編碼引用已同步。詳見 `vChewing-DevLogs/Reqs4LLM/Archive_P201-P300/Reqs_0211-0220.md` Phase 213。
- **落地狀態（2026-09-14，Phase 214 更新——兩個倉的套件身份完全合一）**：兩倉的套件皆名 `LibVanguard`、出貨產品皆名 `Vanguard`（產物皆 `libVanguard.dylib`），聚合靶與其測試靶皆為 `LibVanguard`／`LibVanguardTests`（原 `OSNeutralAssembly`／`OSNeutralAssemblyTests`），**且 `Package.swift` 逐位元組相同、`Sources/` 全等**（`Tests/` 僅差 LibVanguard 倉獨有的 2 個搶救測檔）。manifest 亦改以 `@resultBuilder` 的 `ArrayBuilder` DSL 撰寫。macOS 的套件目錄亦正名為 `Packages/vChewing_OSNeutral_LibVanguard`（SwiftPM 對本地路徑相依之 identity 取自目錄名，故 11 個 consumer manifest 的 `.package(path:)` 與 `.product(…, package:)` 同步）。本文以下各節若引及舊名 `vChewing_OSNeutralAssembly`（含 `…/Sources/OSNeutralAssembly/`），皆為 2026-09-12 至 09-14 各該階段當時的實際名稱。
