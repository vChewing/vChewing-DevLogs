...
- **落地狀態（2026-09-14，Phase 213 更新——同一設計在另一倉的第二次落地）**：`vChewing-LibVanguard` 已整組改為與 `vChewing-macOS/Packages/vChewing_OSNeutralAssembly` 同源的聚合包，出貨單一動態產品 `Vanguard`／`libVanguard.dylib`（`package.name` 取 `LibVanguard`、**product 名取 `Vanguard`**，故此倉不會產生 `libLibVanguard.dylib`；模組名與 target 名一律不變）。此即本研究 §9.3 路線 (d)「另立一個 `.dynamic` product 讓它把整個閉包吸進去」在**非 app 倉**的落地，且該倉原本的 `LibVanguard` 聚合 target ＋ `Sources/_Modules/*` 舊佈局已全部退場。**與 Phase 210 的關鍵差異**：Phase 210 的 dylib 活在 `vChewing-macOS` 的 app 出貨管線內（`Contents/Frameworks`、rpath、簽章、公證），而 LibVanguard 這一側目前**只做到「能編出 dylib 並通過全部單元測試」**——其向前建置能力（消費者改吃此倉）依事主明示屬後續 phase。資源面不受影響：本倉的資源 bundle 名為 `LibVanguard_*.bundle`（SwiftPM 之 bundle 名取套件名），兩處硬編碼引用已同步。詳見 `vChewing-DevLogs/Reqs4LLM/Archive_P201-P300/Reqs_0211-0220.md` Phase 213。
