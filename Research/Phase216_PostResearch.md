...
- 狀態：**術後記錄，P216 已完工。** 完工登記已於 2026-09-15 完成（`vChewing-DevLogs` 之 `942557d`（`P216.`）一筆，動 `DevReqsHistory.md`／`KnowledgeMemo4LLM.md`／`Reqs4LLM/Archive_P201-P300/Reqs_0211-0220.md`／本檔共 4 檔、+762/−7）——**該 hash 其後已失效，見下**。
  - **2026-09-16 更正（hash 盤點）**：該筆已隨 `vChewing-DevLogs` 之 rebase **消失**——`942557d` 與 P217 之 `4429504` 現於任何 ref（含 reflog）皆**無對位可尋**；該倉 P215 以後只有 `239591b`（`P215.`）→ `7add503`（`README // Patch license.`）→ `a046d0c`（`P126-P128.`，即現行 `HEAD`）。兩筆之內容已**摺進該最後一筆**（值鏈：`83b473c` → `d24a0cb` → `a046d0c`；末者係一次 squash，把 P216～P218 之全部文書與 P126～P128 之內容併為一筆）：其 diff 一次動了同樣那五檔（含本檔），即 P216／P217／P218 之登記已合而為一。原句「該筆為現行 `HEAD` 之可達祖先」自此**不成立**（惟各波之結果已使該等登記之內容成為現行 `HEAD` 本身）。
...
| # | commit | 訊息 | 範圍 |
|---|---|---|---|
| 1 | `13097e0` | `SPM // Starting working on Swift 5.10 compilability.` | 6 份版本擇定 manifest（兩個 package 各 3 份）＋ `makefile`：**8 檔、+318/−2** |
| 2 | `ac781c1` | `SwiftExtension // Swift 5.10 compilability.` | `Sources/SwiftExtension/SwiftExtension.swift` ＋ `SwiftFoundationImpl.swift`：**2 檔、+375/−206** |
| 3 | `826a744` | `LibVanguard // Swift 5.10 compilability.` | 聚合體 23 檔：**+315/−299** |
| 4 | `208554e` | `LibVanguard // Patch unit test issues.` | 單元測試結構之整肅：**16 檔、+490/−412**（內容見 §十四） |
| 5 | `1c60407` | `SPM // Block Swift 6.2 and 6.3 toolchains.` | Swift 6.x 收斂為 **6.4+ only**：五個 `Package.swift` 抬版 ＋ 6.2／6.3 封堵檔 ＋ CI pin：**9 檔、+88/−5**（內容見 §13.6） |
...
**現行值一律以 §一 之表為準**（`13097e0`／`ac781c1`／`826a744`／`208554e`／`1c60407`）。**含本檔在內的一切 hash 記載都會隨 amend 失效——判準請以「訊息 ＋ 小節主題」為準，hash 只當線索。**
...
- 本 phase 的正式完工登記**已於同日完成**（`vChewing-DevLogs` 之 `942557d`（`P216.`）——**該 hash 其後已隨該倉之 rebase 消失、於任何 ref 皆無對位可尋；內容現摺於 `a046d0c`（`P126-P128.`；該筆即 `83b473c`／`d24a0cb` 之現值）**；見檔頭之狀態行）。
...
**執行結果（2026-09-15，已完成）**：五個 `Package.swift`（兩倉之聚合體、聚合體之 `Deps/VanguardSwiftExtension`、`vChewing-macOS` 倉根 meta-manifest）之宣告值已抬到 6.4，並在上述各處各鋪 `Package@swift-6.2.swift`／`Package@swift-6.3.swift` 兩份封堵檔；三系統 CI 的 toolchain pin 已同批抬到 ≥6.4。本機驗收：6.3.3 被 `Package@swift-6.3.swift` 的 `#error` 攔下（rc=1）、6.4.0 側 `swift build` rc=0 且 `swift test --no-parallel` **535 項 0 失敗**。
  - **現行 commit 對位**：`vChewing-LibVanguard` = `1c60407`（`SPM // Block Swift 6.2 and 6.3 toolchains.`）、`vChewing-macOS` = `418015a3`（`Repo // Block Swift 6.2 and 6.3 toolchains.`）。（本項執行當時所記之 hash `95a609c`／`d88a423c` 已經 rebase 而不可達；見 §一 之註記。）
...
## 十四、單元測試結構之整肅（`208554e`，2026-09-15 深夜追加）
...
| `SPM // Starting working on Swift 5.10 compilability.` | `13097e0` | 見 `07575402` 之前置批次 |
| `SwiftExtension // Swift 5.10 compilability.` | `ac781c1` | `89af456f` |
| `LibVanguard // Swift 5.10 compilability.` | `826a744` | `3e72cddb` |
| `LibVanguard // Patch unit test issues.` | `208554e` | `bcaebc46`（＝P216／P217 之分水嶺） |
| `SPM // Block Swift 6.2 and 6.3 toolchains.` | `1c60407` | `418015a3`（其 macOS 自身側屬 P217） |
...
- **2026-09-16（三倉 rebase 後之 hash 複測）**：本檔所引之「現行」hash 全數複測更新——`vChewing-LibVanguard` 之第 4、5 筆改為 `45bb0ac`／`6609f5b`，`vChewing-macOS` 之對位改為 `cad648e3`／`588d1b8e`／`48ff88ef`，`vChewing-DevLogs` 之兩筆完工登記改為 `942557d`（`P216.`）／`4429504`（`P217.`）——**惟此兩筆其後已隨該倉之 rebase 消失、於任何 ref 皆無對位可尋；其內容現摺於 `b939f2a`（`P218.`），見檔頭之更正**。**§一 之表與本附錄同步更新**；依 §十二 R12，判準仍以「訊息 ＋ 小節主題」為準。
- **2026-09-16（再一波 rebase 後之 hash 再複測）**：三倉於同日又經一波 rebase——**19 份逐包 `makefile` 之一檔一 commit 沿革被摺回其本源 commit**——故 **§一 之表與本附錄（現行 hash 對照）再次同步更新**：兩表之現行欄已全數改記該次 rebase 後之實測值（`vChewing-LibVanguard`：`13097e0`／`ac781c1`／`826a744`／`208554e`／`1c60407`；`vChewing-macOS` 之對位：`3e72cddb`／`bcaebc46`／`418015a3`／`07575402`）。上一則所記之 `45bb0ac`／`6609f5b`／`cad648e3`／`588d1b8e`／`48ff88ef` 自此皆已失效；`vChewing-DevLogs` 之 `942557d`／`4429504` 仍無任何對位可尋，其內容現摺於 P218 那一筆（`b939f2a` → `83b473c` → `d24a0cb` → `a046d0c`，末者即現行 `HEAD`；檔頭與 §13.4 之引註已同步改記）。判準不變：**以「訊息 ＋ 小節主題」為準**。
- **2026-09-16（第三波；真機 10.9 那一輪之 commit 歸屬入庫後）**：`vChewing-macOS` 之 `5ae0ddc0` 之後再度改寫——新增 `b48b07b5`（`Installer // Patch a null handler assignment to the retry timer.`）**插在 `5ae0ddc0` 之後、原 `Repo // Assembles the app bundle for legacy targets.` 之新值 `5793727b` 之前**，其後兩筆亦隨之改值（`95f6ff84`／`c7812115`）；`vChewing-OSX-legacy` 新增同訊息之對位筆 `b48e1c4f`。故 `b3b19a77`／`3d376bf0`／`75ab0219` 三值自此失效（前二者分別為原先所述之現值），`7f23b1b1` → `b3b19a77` → `5793727b` 之鏈見 `Phase217_SOP.md` 附錄。**本項即現行末次修訂**；其後同一份表格內的「現行」欄仍可能隨下一次 amend／rebase 失效。
- **2026-09-16（第四波；`vChewing-DevLogs` 之 squash）**：該倉把 P126～P128 與 P216／P217／P218 之文書**併為一筆** `a046d0c`（`P126-P128.`，即現行 `HEAD`），故 `d24a0cb`／`049a8bf` 兩值失效（`83b473c` → `d24a0cb` → `a046d0c` 之鏈見 `Phase217_SOP.md` 附錄）；其餘三倉之值不變。此波之後，本倉「該筆不存在、內容摺進那筆」之說法一律以 `a046d0c` 為準。
