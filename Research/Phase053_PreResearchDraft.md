...
> ⚠️ **本文件已過期，僅留供存證。** 正式版本請洽 `Phase053_Research.md`。
> 本文件混合了大量術前假說、中間迭代虛擬碼與術後追記，接手閱讀體驗極差。
> Phase 53 的最終設計決策、實作規格與行為規則已統整於 `Phase053_Research.md`。
...
## 一、被否決 PR 方案分析（Phase053_Ref-MixedInputMode-Architecture-Denied.md）
...
1. **Session 高度耦合**：`rawBuffer` 的宣告、`Ctrl+Shift+M` 切換熱鍵、`isInMixedMode` 狀態旗標、以及「commit 英文後清空 buffer」的行為，全都實作在 `InputSession.swift` / `InputSession_HandleMixedInput.swift` / `InputSession_HandleEvent.swift`，這三個檔案是 macOS 專屬的 Darwin 層，**無法在 Linux 建置目標下編譯**。
...
被否決 PR 把混輸切換做成了 `Ctrl+Shift+M` 熱鍵、`static var isInMixedMode` 旗標——這是一個「模式」（mode）的思路。但在現行架構下，`TypingMethod` 已不適合再擴充：
...
| 切換機制 | `PrefMgrProtocol` 偏好旗標 `mixedAlphanumericalEnabled`，不需要熱鍵切換、不需要修改 FSM |
...
- 更新 vChewing-DevLogs/Reqs4LLM 的 Phase 53 實作紀錄（依 Response Pattern）。
